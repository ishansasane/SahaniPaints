...
function LaborsAttendance() {
  ...
  // Add new labor
  const addLabor = () => {
    const trimmedName = newLaborName.trim();

    if (!trimmedName || !selectedSite) {
      alert("Please enter a labor name and select a site.");
      return;
    }

    const alreadyExists = laborNames.some(
      (labor) => labor.name.toLowerCase() === trimmedName.toLowerCase()
    );

    if (alreadyExists) {
      alert("This labor is already added.");
      return;
    }
